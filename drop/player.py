...
class FeedPlayer:
    ...
    # ── rotation transition (snapshot overlay) ─────────────────────────────

    def _snapshot_current_frame(self):
        """Block briefly while libvlc writes a PNG of the current frame.
        Returns a PIL.Image or None on failure. Used by rotate_video to
        keep visible content on screen during the VLC instance rebuild."""
        if not self._player:
            return None
        try:
            ...
            return None
        import tempfile
        snap_dir = Path(tempfile.gettempdir()) / "drop_rot"
        try:
            snap_dir.mkdir(exist_ok=True)
        except Exception:
            return None
        snap_path = snap_dir / f"rot_{int(time.time() * 1000)}.png"
        try:
            # 0,0 = use VLC's natural display size. video_take_snapshot is
            # async — it queues the request and returns immediately.
            self._player.video_take_snapshot(0, str(snap_path), 0, 0)
        except Exception:
            return None
        # Poll for the file briefly. 80ms ceiling so a slow VLC doesn't
        # turn the rotate button into a long blocking call.
        for _ in range(16):
            if snap_path.exists() and snap_path.stat().st_size > 256:
                try:
                    img = Image.open(str(snap_path))
                    img.load()  # detach from the file so we can let it linger
                    return img
                except Exception:
                    return None
            time.sleep(0.005)
    ...
    def _build_rotation_overlay(self, pil_img, target_rotation):
        """Place a Tk Label on top of the surface holder, showing the given
        frame at 0° rotation. Returns a state dict the animator updates."""
        try:
            from PIL import Image as _Image, ImageTk
        except Exception:
            return None
        holder = self.surface_holder
        sw = max(self.surface.winfo_width(), 2)
        sh = max(self.surface.winfo_height(), 2)
        sx = self.surface.winfo_x()
        sy = self.surface.winfo_y()
        # Resize the snapshot to match the surface so the overlay overlaps
        # the VLC output exactly — no visible seam at the start of the
        # animation.
        try:
            base = pil_img.resize((sw, sh), resample=_Image.LANCZOS)
        except Exception:
            base = pil_img
        photo = ImageTk.PhotoImage(base)
        # Label bg matches the surface_holder's frame color (BG) so the
        # rotation's wedge corners — which appear past 0°/90°/180°/270° as
        # the expanded bbox grows beyond the rotated content — blend into
        # the surrounding chrome instead of standing out as black bars
        # against the slightly-lighter holder background.
        label = tk.Label(holder, image=photo, bg=BG,
                         borderwidth=0, highlightthickness=0)
        label.image = photo
        label.place(x=sx, y=sy, width=sw, height=sh)
        label.lift()
        # Make sure no stale center_hint text (PAUSED / seek indicator /
        # mute toast) flashes through the surface during the rebuild.
        try: self.center_hint.place_forget()
        except Exception: pass
        # Hide the title overlay if it was expanded — otherwise the full
        # title text would peek over the rotating frame.
    ...
    def _animate_overlay_rotation(self, overlay, end_angle, ms=180):
        """Tween the overlay from 0° to end_angle over ms milliseconds.

        Frames are pre-rendered ONCE up front, so each tick is just a
        configure(image=…). Doing rotate()+resize() per tick on an
        800×450 snapshot is ~15ms of CPU work — enough that the animation
        drops below 30fps and the user sees a visibly jittery spin. The
        pre-render trades ~80ms of blocking work up front for a smooth
        60fps playback that finishes well before VLC's first decoded
        frame arrives anyway."""
        if overlay is None or not overlay.get("alive"):
            return
        try:
            from PIL import Image as _Image, ImageTk
        except Exception:
        ...
        hw = max(holder.winfo_width(), 1)
        hh = max(holder.winfo_height(), 1)

        def ease(t):
            return 1 - (1 - t) ** 3

        # 5 keyframes ≈ one frame every 36ms over 180ms. Few enough that
        # pre-render stays under 100ms total, many enough that the spin
        # still reads as continuous motion thanks to the eye filling in
        # between frames at ~25fps perceived rate.
        N = 5
        keyframes = []
        for i in range(N + 1):
            # Negative angle for CW visual when end_angle > 0 — PIL rotates
            # CCW by default; matching VLC's `transform-type=90` (CW) means
            # we flip the sign here.
            angle = -end_angle * ease(i / N)
            # fillcolor=BG paints the expanded-bbox wedges with the exact
            # same color as the surrounding player frame.
            rotated = overlay["image"].rotate(
                angle, resample=_Image.BILINEAR, expand=True, fillcolor=BG,
            )
            # Aspect-fit to holder so the spinning frame never spills past
            # the holder bounds — without this, intermediate angles get
            # clipped at the chrome edge and the visible slice looks like
            # a thumbnail jammed in a corner.
            rw0, rh0 = rotated.size
            scale = min(hw / rw0, hh / rh0, 1.0)
            if scale < 1.0:
            ...
            keyframes.append((photo, rotated.size[0], rotated.size[1]))

        # Stash on overlay so PhotoImage refs stay alive for the duration
        # of the animation. Without this, Tk's PhotoImage refcount drops
        # to 0 the moment the loop variable goes out of scope and the
        # label renders a blank image.
        overlay["keyframes"] = keyframes

        start = time.time()
        duration = max(0.05, ms / 1000.0)

        def tick():
            if not overlay.get("alive"):
                return
            t = (time.time() - start) / duration
            if t > 1.0: t = 1.0
            idx = min(N, int(t * N))
            photo, rw, rh = keyframes[idx]
            try:
                overlay["label"].configure(image=photo)
                overlay["label"].image = photo
                overlay["label"].place(
                    x=(hw - rw) // 2, y=(hh - rh) // 2,
                    width=rw, height=rh,
                )
